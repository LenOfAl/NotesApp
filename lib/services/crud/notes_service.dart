...
import 'package:comingsoon/extensions/list/filter.dart';
...
class NotesService {
  ...
  Future<DatabaseNote> createNote({required DatabaseUser owner}) async {
    ...
    //make sure owner exists in database corrent id
  ...
  Future<DatabaseUser> createUser({required String email}) async {
    ...
    final userId = await db.insert(userTable, {
      DatabaseUser.emailColumn: email.toLowerCase(),
    });
    return DatabaseUser(
      id: userId,
      email: email,
  ...
  Database _getDatabaseOrThrow() {
    final db = _db;
    if (db == null) {
      throw DatabaseIsNotOpenExcepetion();
    } else {
      return db;
    }
  }
...
const createNotesTable = '''CREATE TABLE IF NOT EXISTS "note"(
      ...
      PRIMARY KEY("user_id" REFERENCES "user"("id")),
      PRIMARY KEY("id" AUTOINCREMENT);
);''';
